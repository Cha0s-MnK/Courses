...
def eq_u_0(u_0, a):
    return np.exp(-u_0**2) * u_0**4 - a / np.sqrt(np.pi)
...
def main():
    ...
    plt.title(r'Voigt function $H(a, u)$ for different values of $a$')
    plt.xlabel(r'$u$')
    plt.ylabel(r'$H(a, u)$')
    plt.legend()
    ...
    # Calculate the Voigt function and its approximation for limit a and plot the results
    ...
    plt.figure(figsize=(figsizeX, figsizeY), dpi=512)
    plt.plot(us, H, label='numerical Voigt function', color='blue')
    plt.plot(us, H_approx, linestyle='--', label='approximate Voigt function', color='red')

    # plot settings and save the plot as a PNG file
    plt.title(r'Voigt function comparison for $a = 0.01$')
    plt.xlabel(r'$u$')
    plt.ylabel(r'$H(a, u)$')
    plt.legend()
    plt.grid(True, which="both", linestyle=':')
    ...
    # plot settings and save the plot as a PNG file
    plt.title(r'Relative error of the approximate Voigt function for $a = 0.01$')
    plt.xlabel(r'$u$')
